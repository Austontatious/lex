...
def extract_surface_cues(text: str) -> Dict[str, float]:
    ...
    cues["all_caps"] = sum(1 for w in text.split() if w.isupper() and len(w) > 2) / max(
        1, len(text.split())
    )
    # NOTE: Real misspell detection needs spellchecker, but here's a fast proxy
    cues["misspell"] = sum(
        1 for w in text.split() if len(w) > 3 and not w.islower() and w.isalpha()
