...
_client = chromadb.Client(chromadb.config.Settings(persist_directory=CHROMA_PATH))
...
def archive_context_to_chroma(entries: List[Dict], session_id: str):
    ...
    _collection.add(documents=documents, embeddings=embeddings, metadatas=metadatas, ids=ids)
...
def semantic_search(query: str, k: int = 5) -> List[Dict]:
    ...
    for id_, doc, meta in zip(results["ids"][0], results["documents"][0], results["metadatas"][0]):
        matches.append({"id": id_, "text": doc, "metadata": meta.get("metadata", {})})
    return matches
