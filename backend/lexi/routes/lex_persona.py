...
DESCRIBE_PATTERN = re.compile(
    r"\b(describe|what do you look like|show me your look|how do you look)\b", re.IGNORECASE
)
...
async def add_trait(request: Request) -> Dict[str, Any]:
    ...
    narration = (
        "Here's your look! If you'd like to tweak anything, let me know."
        if ready
...
async def get_persona() -> Dict[str, Any]:
    ...
    try:
        ...
        if avatar_file.exists():
            avatar = f"{AVATAR_URL_PREFIX}/{avatar_name}"
        else:
            avatar = STARTER_AVATAR_PATH
        ready = not get_missing_fields(traits)
        return {"traits": traits, "certainty": 1.0 if ready else 0.8, "image_path": avatar}
    except Exception as exc:
        logger.warning("Failed loading persona: %s", exc)
        return {"traits": {}, "certainty": 0.0, "image_path": STARTER_AVATAR_PATH}
