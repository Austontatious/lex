...
class NowSettings(BaseSettings):
    ...
    SUMMARIZER_ENDPOINT: AnyHttpUrl = Field(
        default_factory=lambda: _env_or_default("SUMMARIZER_ENDPOINT", "http://127.0.0.1:8008/v1/chat/completions")
    )
    SUMMARIZER_MODEL: str = Field(default_factory=lambda: _env_or_default("SUMMARIZER_MODEL", "Lexi"))

    NOW_REFRESH_MINUTES: int = Field(default_factory=lambda: int(os.getenv("NOW_REFRESH_MINUTES", "30")))
    NOW_MAX_ITEMS_PER_SOURCE: int = Field(default_factory=lambda: int(os.getenv("NOW_MAX_ITEMS_PER_SOURCE", "20")))
    NOW_TOP_N_DEFAULT: int = Field(default_factory=lambda: int(os.getenv("NOW_TOP_N_DEFAULT", "8")))
    REDIS_URL: Optional[str] = Field(default_factory=lambda: os.getenv("REDIS_URL", "redis://127.0.0.1:6379/0"))

    DEFAULT_INTERESTS: List[str] = Field(
        default_factory=lambda: [s.strip() for s in os.getenv("DEFAULT_INTERESTS", "movies,tv,technology,ai").split(",") if s.strip()]
    )
