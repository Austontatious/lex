...
Env:
  ...
  LEX_IMAGE_DIR     (default: <repo>/lexi/static/lexi/avatars)
"""
...
IMAGE_DIR = Path(os.getenv("LEX_IMAGE_DIR", str(LEX_ROOT / "static" / "lexi" / "avatars")))
...
FLUX_DIFFUSION_DIR = Path(os.getenv("FLUX_DIFFUSION_DIR", str(FLUX_MODELS_DIR / "diffusion_models")))
FLUX_TEXT_ENCODER_DIR = Path(os.getenv("FLUX_TEXT_ENCODER_DIR", str(FLUX_MODELS_DIR / "text_encoders")))
...
FLUX_T5XXL = Path(os.getenv("FLUX_T5XXL", str(FLUX_TEXT_ENCODER_DIR / "t5xxl_fp8_e4m3fn.safetensors")))
...
def _flux_variant_paths(variant: Optional[str]) -> FluxPaths:
    ...
    if not paths:
        raise ValueError(f"Unsupported Flux variant '{variant}'. Available: {', '.join(sorted(FLUX_VARIANTS))}")
    # verify files exist
...
_MAJOR_EDIT_TOKENS = {
    "hair": ["long hair", "short hair", "bob", "pixie", "ponytail", "bun", "bangs", "fringe", "updo"],
    "body": ["curvy", "hourglass", "athletic", "slim thick"],
    "outfit": ["jean shorts", "shorts", "jacket", "dress", "gown", "hoodie", "coat", "skirt"],
}
OUTFIT_KEYWORDS = {
    "outfit", "shirt", "tshirt", "t-shirt", "tee", "top", "jeans", "shorts",
    "skirt", "dress", "hoodie", "jacket", "leggings", "stockings", "thigh-high",
    "boots", "heels", "sneakers", "jean shorts", "denim shorts", "crop top",
}
    ...
    if nsfw:
        for name in [
            ...
            *NSFW_MERGES,
            ...
            if _exists_ckpt(name): return name
...
def _chain_loras(g: Dict[str, Any], model_key: list, clip_key: list,
                 loras: list[dict], prefix: str) -> tuple[list, list]:
    """
...
def _identity_seed_from_traits(traits: Dict[str, str]) -> int:
    ...
    base_keys = {k: traits[k] for k in sorted(traits.keys()) if any(t in k for t in ("hair", "eyes", "face", "skin"))}
...
def _post_graph(graph: Dict[str, Any]) -> str:
    r = requests.post(f"{COMFY_URL}/prompt", json={"prompt": graph, "client_id": "lexi"}, timeout=60)
    r.raise_for_status()
...
def _download_image(filename: str, subfolder: str = "", ftype: str = "output", dst: Optional[Path] = None) -> Path:
    ...
    for n in flow.get("nodes", []):
        if n.get("type") == "CheckpointLoaderSimple":
            ...
            if isinstance(vals, list) and vals:
                if vals[0] and ("refiner" in str(vals[0]).lower() or "checkpoints/sd_xl_refiner" in str(vals[0])):
                    if ref_file:
...
def _set_empty_latent_size(flow: Dict[str, Any], w: int, h: int) -> None:
    ...
    if "nodes" not in flow:
        for node in flow.values():
            if isinstance(node, dict) and node.get("class_type") in ("EmptyLatentImage", "EmptyLatentImageNode"):
...
def _set_text_prompts(flow: Dict[str, Any], positive: str, negative: str) -> None:
    ...
    """
    neg_markers = ("cartoon, anime", "bad anatomy", "lowres", "watermark", "3d render", "harsh shadow")
    # Editor-style
    for n in flow.get("nodes", []):
    ...
    if "nodes" not in flow:
        ...
        for node in flow.values():
            if isinstance(node, dict) and node.get("class_type") == "CLIPTextEncode":
                ...
                (node.setdefault("inputs", {}))["text"] = negative if any(m in old for m in neg_markers) else positive

def _set_sampler_params(flow: Dict[str, Any], *, steps: int, cfg: float, seed: int, denoise: Optional[float] = None) -> None:
    """
...
def _ensure_latent_upscale(flow: Dict[str, Any], scale_by: float) -> None:
    ...
    if "nodes" not in flow:
        for node in flow.values():
            if isinstance(node, dict) and node.get("class_type") in ("LatentUpscaleBy", "ImageScaleBy"):
                node.setdefault("inputs", {})["scale_by"] = float(scale_by)

# ------------------------- Aesthetic tuning -------------------------

def _tune_lora_strengths(flow: Dict[str, Any], *, overrides: Optional[Dict[str, Tuple[float, float]]] = None,
                         default_unet: float = 0.5, default_clip: float = 0.25) -> None:
    """
...
def _apply_realistically_unreal_profile(flow: Dict[str, Any], *, is_face: bool) -> None:
    ...
    _tune_lora_strengths(flow, overrides=overrides, default_unet=0.38 if is_face else 0.25, default_clip=0.16)
    if not is_face:
...
def _has_image_sink(payload: Dict[str, Any]) -> bool:
    ...
    if "nodes" in payload:
        ...
        return any((n.get("type") in ("SaveImage", "PreviewImage", "ImageSave", "ImageOutput")) for n in nodes)
    # API-prompt style
    return any((v.get("class_type") in ("SaveImage", "PreviewImage", "ImageSave")) for v in payload.values() if isinstance(v, dict))
...
def _attach_save_if_missing(flow: Dict[str, Any]) -> None:
    ...
    if "nodes" not in flow:
        has_save = any(isinstance(v, dict) and v.get("class_type") == "SaveImage" for v in flow.values())
        if has_save:
            return
        # find a likely image source: prefer ImageScaleBy, then VAEDecode, then ImageComposite
        candidates = [k for k, v in flow.items() if isinstance(v, dict) and v.get("class_type") in ("ImageScaleBy", "VAEDecode", "ImageComposite")]
        if not candidates:
            return
        try:
...
def _use_face_workflow(*, prompt: str, negative: str, w: int, h: int, steps: int, cfg: float,
                       seed: int, refiner_strength: float, upscale_factor: float,
                       base_file: str, refiner_file: Optional[str]) -> Dict[str, Any]:
    """
    ...
    _set_sampler_params(flow, steps=steps, cfg=cfg, seed=seed, denoise=None)  # face graph handles denoise internally
    _ensure_latent_upscale(flow, upscale_factor if USE_UPSCALE else 1.0)
    _apply_realistically_unreal_profile(flow, is_face=True)
    return flow

def _use_body_workflow(*, prompt: str, negative: str, w: int, h: int, steps: int, cfg: float,
                       seed: int, denoise: float, src_uploaded_name: str,
                       base_file: str, refiner_file: Optional[str],
                       upscale_factor: float) -> Dict[str, Any]:
    """
...
def _graph_sdxl_txt2img(prompt: str, negative: str, w: int, h: int, steps: int, cfg: float,
                        seed: int, use_refiner: bool, refiner_strength: float,
                        upscale_factor: float,
                        base_file: str,                      # <<< NEW
                        refiner_file: Optional[str] = None,  # <<< NEW
                        style: str = "realistic", nsfw: bool = False) -> Dict[str, Any]:
    """
    ...
    g["t_enc_base"] = {"class_type": "CLIPTextEncode",
                       "inputs": {"clip": clip_in, "text": prompt}}
    g["t_neg_base"] = {"class_type": "CLIPTextEncode",
                       "inputs": {"clip": clip_in, "text": negative}}
    g["latent"] = {"class_type": "EmptyLatentImage",
                   "inputs": {"width": w, "height": h, "batch_size": 1}}
    g["ks_base"] = {"class_type": "KSampler",
                    "inputs": {
                        "model": model_in,
    ...
    if use_refiner and refiner_file:
        ...
        if _exists_ckpt(ref_ckpt_name):
            g["ckpt_ref"] = {"class_type": "CheckpointLoaderSimple",
                             "inputs": {"ckpt_name": ref_ckpt_name}}
            g["t_enc_ref"] = {"class_type": "CLIPTextEncode",
                              "inputs": {"clip": ["ckpt_ref", 1], "text": prompt}}
            g["t_neg_ref"] = {"class_type": "CLIPTextEncode",
                              "inputs": {"clip": ["ckpt_ref", 1], "text": negative}}
            g["ks_ref"] = {"class_type": "KSampler",
                           "inputs": {
                               "model": ["ckpt_ref", 0],
    ...
    if USE_UPSCALE and upscale_factor and upscale_factor > 1.01:
        g["lat_up"] = {"class_type": "LatentUpscale",
                       "inputs": {
                           "samples": prev_latent,
    ...
    g["decode"] = {"class_type": "VAEDecode",
                   "inputs": {"samples": prev_latent, "vae": prev_vae}}
...
def _graph_sdxl_img2img(
    ...
    g["t_enc"] = {"class_type": "CLIPTextEncode",
                  "inputs": {"clip": clip_handle, "text": prompt}}
    g["t_neg"] = {"class_type": "CLIPTextEncode",
                  "inputs": {"clip": clip_handle, "text": negative}}
...
def _graph_sdxl_inpaint(
    ...
    g["t_pos"] = {"class_type": "CLIPTextEncode",
                  "inputs": {"clip": ["ckpt", 1], "text": prompt}}
    g["t_neg"] = {"class_type": "CLIPTextEncode",
                  "inputs": {"clip": ["ckpt", 1], "text": negative}}

    # Source image + mask
    g["img"] = {"class_type": "LoadImage",
                "inputs": {"image": source_filename, "choose file to upload": "image"}}
    g["mask"] = {"class_type": "LoadImageMask",
                 "inputs": {"image": mask_filename, "choose file to upload": "image"}}

    # Encode to latent
    g["enc"] = {"class_type": "VAEEncode",
                "inputs": {"pixels": ["img", 0], "vae": ["ckpt", 2]}}

    # Apply the mask as noise mask for inpainting
    # (white = editable area by default; invert if needed)
    g["noise_mask"] = {"class_type": "SetLatentNoiseMask",
                       "inputs": {
                           "samples": ["enc", 0],
                           "mask": ["mask", 0],
    ...
    *,
    ...
    if mode == "img2img":
        if source_path is None or not source_path.exists():
            raise ValueError("img2img requested but no valid source image was provided for Flux backend.")
        if _is_in(IMAGE_DIR, source_path) and source_path.name != base_path.name and not allow_feedback_loop:
            raise ValueError(
    ...
    if not images:
        return {"ok": False, "error": "No images returned from ComfyUI (Flux backend)", "prompt_id": pid}

    first = images[0]
    out = _download_image(first.get("filename", ""), first.get("subfolder", ""), first.get("type", "output"))
    final_out = _finalize_generated_image(out, base_path, force_output_to_base)

    base_url = public_base_url
    public = (
        f"{base_url}/static/lexi/avatars/{final_out.name}"
        if base_url
        else f"/static/lexi/avatars/{final_out.name}"
    )
    return {
        "ok": True,
        "file": str(final_out),
...
def generate_avatar_pipeline(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        backend = str(kwargs.get("backend") or kwargs.get("model") or SD_BACKEND or "sdxl").strip().lower()
        ...
        intent_text = (prompt or " ".join([
            str((traits or {}).get(k, "")) for k in ("outfit","style","hair","pose","vibe")
        ])).lower()
        wants_outfit = any(k in intent_text for k in OUTFIT_KEYWORDS) or bool(traits and traits.get("outfit"))
        ...
        if kwargs.get("nsfw") or any(x in p_low for x in ("lingerie", "thigh-high", "stockings", "nsfw")):
            unblock = {"latex", "shiny clothing", "stockings", "thigh-highs", "sheer", "see-through"}
            toks = [t.strip() for t in neg_full.split(",") if t.strip().lower() not in unblock]
            neg_full = ", ".join(toks)
        ...
        if backend != "flux":
            ...
            if sel.variation:
                ...
                def _jit(val, delta):
                    return val if delta in (None, 0) else (val + random.uniform(-float(delta), float(delta)))
                ...
                if "denoise" in kwargs:
                    kwargs["denoise"] = max(0.0, min(1.0, _jit(float(kwargs["denoise"]), jitter.get("denoise"))))
        else:
            sel = None
        ...
        if backend == "flux":
            ...
            flux_denoise = kwargs.get("flux_denoise") if kwargs.get("flux_denoise") is not None else kwargs.get("denoise")
            log.info("[Lexi SD][flux] mode=%s seed=%s variant=%s steps=%d cfg=%.2f guidance=%.2f width=%d height=%d",
                     mode, seed, (flux_variant or DEFAULT_FLUX_VARIANT),
                     steps, cfg_scale, (flux_guidance or FLUX_DEFAULT_GUIDANCE),
                     width, height)
        ...
        log.info(
            ...
            task, mode, seed, base_file, ref_file,
            (kwargs.get("denoise") if mode == "img2img" else "-"),
            cfg_scale, steps, lora_names
        )
        ...
        if mode == "txt2img":
            ...
            if not sp or not sp.is_file():
                raise ValueError("img2img requested but no valid source image was provided (expected lexi_base.png).")

            # Safety: allow reprocessing the fixed base, but block non-base outputs unless explicitly allowed.
            if _is_in(IMAGE_DIR, sp) and sp.name != base_name and not bool(kwargs.get("allow_feedback_loop", False)):
                raise ValueError("Refusing to reprocess a freshly generated avatar (non-base). Use lexi_base.png or pass allow_feedback_loop=True.")
        ...
        base_url = PUBLIC_BASE_URL
        public = (f"{base_url}/static/lexi/avatars/{final_out.name}" if base_url else f"/static/lexi/avatars/{final_out.name}")
        return {
            "ok": True,
            "file": str(final_out),
            "url": public,
