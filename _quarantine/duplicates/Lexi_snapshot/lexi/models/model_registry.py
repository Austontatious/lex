...
class ModelRegistry:
    ...
    def select(self, task: str, overrides: Optional[dict] = None) -> Selection:
        ...
        if "base" in overrides:                 base_file = overrides["base"]
        if "refiner" in overrides:              refiner_file = overrides["refiner"]
        if "cfg" in overrides:                  cfg = overrides["cfg"]
        if "steps" in overrides:                steps = overrides["steps"]
        if "refiner_strength" in overrides:     refiner_strength = overrides["refiner_strength"]
        if "loras" in overrides:                loras_out = overrides["loras"]
        if "denoise" in overrides:              denoise = overrides["denoise"]
        if "variation" in overrides:            variation = overrides["variation"]
