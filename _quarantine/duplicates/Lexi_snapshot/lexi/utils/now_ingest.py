...
async def _fetch_with_retry(name: str, call: Callable[[], Awaitable[httpx.Response]]) -> httpx.Response:
    delay = 0.5
...
async def ingest_tvmaze() -> List[NowItem]:
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        for row in response.json()[: settings_now.NOW_MAX_ITEMS_PER_SOURCE]:
            ...
            url = (row.get("url") or (row.get("show") or {}).get("url")) or "https://www.tvmaze.com/"
            ...
            out.append(
                NowItem(
                    ...
                    source="tvmaze",
                    url=url,
                    title=f"{show}: {episode}" if show and episode else (show or episode or "TV episode"),
                    published_at=dt,
                    image=None,
                    category="tv",
                    tags=["episode"],
...
async def refresh_now_feed() -> None:
    ...
    results = await asyncio.gather(ingest_tmdb(), ingest_tvmaze(), ingest_google_news(), return_exceptions=True)
    ...
    log_now(f"refresh_now_feed: done (movies={len(movies)} tv={len(tv_items)} news={len(news_items)})")
