...
SLANG = {
    "kinda", "sorta", "gonna", "wanna", "tho", "tho.", "imo", "irl",
    "btw", "lol", "lmao", "idk", "ikr", "brb", "af", "tbh", "omg",
}
...
def analyze_user_style(
    ...
    avg_sentence_len = statistics.mean([max(1, len(_words(s))) for s in sents]) if sents else len(words)
    ...
    reading_grade = max(1.0, 0.39 * avg_words_per_sentence + 11.8 * (avg_chars_per_word / 5.0) - 15.59)
    ...
    chaos = max(0.0, min(1.0, profanity_density * 2 + dashes_per_100w * 0.02 + ellipses_ratio * 0.5))
...
def make_style_directives(profile: Dict, persona_traits: Optional[Dict] = None) -> str:
    p = profile
    sent_len_desc = (
        "short" if p["avg_sentence_len"] < 9 else
        "medium" if p["avg_sentence_len"] < 17 else
        "long"
    )
    ...
    lower_b = "high" if p["lowercase_pref"] > 0.5 else ("medium" if p["lowercase_pref"] > 0.2 else "low")
    ...
    if persona_traits:
        extras = []
        if persona_traits.get("playful"): extras.append("Favor playful metaphors when appropriate.")
        if persona_traits.get("analytical"): extras.append("Prefer crisp structure and explicit reasoning steps.")
        if persona_traits.get("romance"): extras.append("Weave gentle affection into responses when invited.")
        if extras:
...
if __name__ == "__main__":
    ...
    mock_gen = "Sure — soft pastels, rounded edges, and cozy textures. I’ll make it feel playful and warm."
