...
#from .microtransactions import handle_premium_request, get_storefront_menu, purchase_product
...
from ..routes.love_loop import record_user_message, maybe_ask_question
...
#        result = handle_premium_request(user_msg, user_credits)
#
#        if result['trigger']:
#            product = result['product']
#            prompt = result['message']  # This is your LLM seed
#            # Example: Let Lexi “pitch” it naturally
#            reply = generate_reply(f"{prompt} Make it playful, flirty, and a little coy.")
#            return reply
    ...
    def build_avatar_url(filename: str) -> str:
        avatar_file = Path(__file__).resolve().parent.parent / "static" / "lex" / "avatars" / filename
        if avatar_file.exists():
            timestamp = int(os.path.getmtime(avatar_file))
            return f"/static/lex/avatars/{filename}?v={timestamp}"
        else:
            return f"/static/lex/avatars/{filename}"  # fallback if file missing
