...
from ..identity.normalize import normalize_handle
from ..user_identity import identity_store
...
async def account_bootstrap(
    ...
    # AccountStore stores username/email + disclaimer state; IdentityStore handles bindings.
    account_store = store
    ...
    if "@" in raw_identifier:
        ...
        user = account_store.get_by_username(username)

    # entry_mode == "new"
    if entry_mode == "new":
        if user is None:
            try:
                user = account_store.create_user(username=username, email=email)
            ...
            handle_norm = normalize_handle(raw_identifier)
            identity.ensure_user(str(user["id"]))
            if handle_norm:
                identity.upsert_handle(handle_norm, str(user["id"]), raw_identifier)
                identity.increment_handle_use(handle_norm, str(user["id"]))
            if device_id:
                identity.bind_device(device_id, str(user["id"]), handle_norm=handle_norm or None)
            identity.upsert_session(session_id, str(user["id"]))
            log.info(
                "account_bootstrap",
                extra={
...
async def disclaimer_ack(
    ...
    account_store = store
    identity = identity_store()
    user = account_store.get_by_id(payload.user_id)
    if not user:
    ...
    if payload.accepted:
        try:
            user = (
                account_store.mark_disclaimer(
                    payload.user_id, accepted=True, version=payload.version
                )
                or user
            )
        except Exception as exc:
    ...
    identity.ensure_user(payload.user_id)
    device_id = getattr(request.state, "device_id", None)
    if device_id:
        identity.bind_device(device_id, payload.user_id)
    identity.upsert_session(session.session_id, payload.user_id)
