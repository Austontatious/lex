...
async def avatar_endpoint(
    response: Response, request: Request, req: Optional[ChatRequest] = None
) -> Dict[str, Any]:
    if request.method.upper() == "GET":
        avatar_url = await _per_ip_avatar_or_fallback(request)
        response.headers["Cache-Control"] = "no-store"
        return {"ok": True, "url": avatar_url, "avatar_url": avatar_url}
    ...
    avatar_url = await _per_ip_avatar_or_fallback(request)
    response.headers["Cache-Control"] = "no-store"
    return {"ok": True, "url": avatar_url, "avatar_url": avatar_url, "meta": result.get("meta", {})}
